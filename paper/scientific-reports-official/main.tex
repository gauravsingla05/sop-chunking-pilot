...
\title{Position-Aware Chunking for Industrial
Standard-Operating-Procedure Automation: Preserving Step Granularity
and Preconditions in LLM-Extracted Workflows}

\author[1,*]{Gourav Singla}
\affil[1]{Independent Researcher, Cumming, GA, USA}
\affil[*]{gouravsingla05@gmail.com}

\keywords{Position-aware chunking, SOP-to-workflow automation, Large
language models, Document understanding, Safety-critical automation}

\begin{abstract}
Large-language-model pipelines that turn industrial Standard Operating
Procedures (SOPs) into executable workflows must first chunk the
source document. Chunking strategies designed for retrieval-augmented
question answering treat document order as incidental, but SOPs invert
this assumption: step sequence and precondition--step adjacency are
load-bearing. This work formalises three structural-fidelity
properties for chunking procedural text (step-order preservation,
precondition adjacency, safety-constraint attachment), introduces
step-count fidelity as the counterpart to Kendall's $\tau$ that
exposes the over-segmentation pathology under which order-only
metrics reward chunkers that fragment the source into a long, noisy
step list, and reports a 26-document pilot on public US-government
SOPs that compares five chunkers (fixed-size, recursive, semantic,
layout-aware, and a rule-based position-aware chunker, PAC) while
holding the LLM extractor and prompt constant. Two findings are
...
subset, and the chunker ranking under LLM-based evaluation is partly
extractor-dependent. The latter is demonstrated by a cross-extractor
robustness check in which the entire pipeline was re-run with OpenAI
gpt-4o-mini in place of Gemini Flash as both per-chunk extractor and
oracle: on the 15 documents where both oracles returned non-empty
graphs, the step-count-fidelity rank order changes (semantic chunking
takes the lead and PAC drops to third). Within this frame, PAC is
presented as an existence proof that a transparent rule-based
...
Gemini-based pilot (0.46 and 0.44 versus 0.32--0.42 and 0.32--0.38
for the four baselines), the two strongest comparisons against the
simplest baselines reach uncorrected $p=0.046$ and $p=0.040$ but
neither would survive a family-wise correction across the 24 paired
tests run, and PAC's lead is concentrated in the 5 Nuclear Regulatory
Commission inspection procedures whose dense step markers match PAC's
detection regime. The same-model coupling between the per-chunk
extractor and the whole-document oracle is named explicitly as the
deepest remaining confound; a hand-annotated gold subset is required
to settle the underlying empirical question. The corpus manifest, both
extractor runs, pipeline, and a reference PAC implementation are
...
\noindent{\sffamily\small\textbf{Keywords:} Position-aware chunking;
SOP-to-workflow automation; Large language models; Document
understanding; Safety-critical automation.}
\vskip12pt
...
\draw[dashed, thick] (axis cs:fixed,9) -- (axis cs:PAC,9)
  node[pos=0.05, above, font=\scriptsize\itshape] {gold IP\,69009};
\draw[dashed, thick] (axis cs:fixed,28) -- (axis cs:PAC,28)
  node[pos=0.05, above, font=\scriptsize\itshape] {gold IP\,69012};
\end{axis}
\end{tikzpicture}
...
\State \emph{Pass 1: step-boundary detection.} Tokenize $D$ into
       ...
       numbered, lettered, or keyword (``Step $N$'') list marker, or
       opens with an imperative verb from a fixed SOP action lexicon.
\State \emph{Pass 2: governing-clause attachment.} For each candidate
       precondition or safety clause, resolve the step it governs by
       (a) explicit reference (``before step 4''), (b) proximity to
       the next imperative span, or (c) section scope. Bind the clause
       to its target step.
\State \emph{Pass 3: cross-reference closure.} For each step
       containing a reference (``see step 9''), if the referenced step
       would land in a different chunk, either grow the current chunk
       to include the referenced step or inline a one-sentence summary
       of the referenced step in the current chunk.
\State \emph{Pass 4: size enforcement.} Pack the resulting (step,
       attached clauses) units greedily into chunks of at most $T$
       tokens. When a unit exceeds the cap, split at the lowest-cost
       boundary that respects P1--P3: prefer section breaks, then
       step-group boundaries, then paragraph breaks within non-safety
       step bodies.
...
G.S.\ conceived the study, formalised the structural-fidelity
properties, implemented the position-aware chunker and the evaluation
pipeline, assembled the corpus, ran the experiments, analysed the
...
\textbf{Funding:} This research received no external funding; it was

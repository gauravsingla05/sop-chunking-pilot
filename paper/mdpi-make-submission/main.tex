...
a 26-document pilot of public US-government SOPs (OSHA, NRC, EPA,
CDC/NIOSH; the corpus is a public-domain proxy for the motivating
industrial-SOP domain). The first contribution is to pair step-count
fidelity with Kendall's $\tau$ as companion metrics for chunking
procedural text, on the observation that $\tau$ is computed only over
aligned gold steps and is therefore blind to spurious extra steps,
while a count-fidelity metric is not. The second is direct empirical
evidence that LLM-as-oracle chunker rankings are unstable: replacing
the extractor and oracle with OpenAI~gpt-4o-mini in place of
Gemini~Flash changes the step-count-fidelity rank order of five
chunkers on the 15 documents for which both oracles return non-empty
graphs, the Gemini oracle identifies at least one precondition on all
26 documents and the gpt-4o-mini oracle identifies zero on every
document, and an idempotency control shows that the oracle's reference
count $n^{*}$ varies 6$\times$ with prompt phrasing on small documents.
Under this evaluation regime the absolute fidelity numbers are
therefore not interpretable in level, only possibly in rank, and the
rank itself moves between extractors. A matched-prompt multiplicity
isolation on the same small documents attributes a 9--16$\times$
inflation in declared step counts to chunked multi-call extraction
alone, independent of prompt asymmetry, so the four- to nine-fold
chunked-versus-oracle ratio reported in the body of this paper is a
conservative under-report of the multiplicity-driven inflation rather
than evidence of it. A transparent rule-based position-aware chunker
(PAC) is presented as a worked example of a chunker that targets the
three structural-fidelity properties (step-order preservation,
precondition adjacency, safety-constraint attachment) by construction;
PAC produces the highest mean step-count fidelity on the Gemini-based
pilot with two uncorrected pairwise $p<0.05$ comparisons, but no PAC
superiority claim survives multiplicity correction, the measured
advantage is a single-stratum NRC effect that is partly circular with
PAC's rule set, and PAC's count fidelity does not replicate under
gpt-4o-mini. The corpus manifest, both extractor runs, the
...
prompt-symmetric re-evaluation can be conducted on the same documents.}
...
transparent rule-based chunker that targets the three
structural-fidelity properties by construction. PAC produces the
highest mean step-count fidelity on the Gemini-based pilot, but no
PAC superiority claim survives multiplicity correction across the 48
paired tests run; the measured advantage is a single-stratum effect
on the NRC inspection procedures, is partly circular with PAC's
rule set and the oracle's whole-document regime, and does not replicate
under gpt-4o-mini. The matched-prompt multiplicity-isolation control
reported in Section~\ref{sec:robustness} attributes 9 to 16-fold
inflation of declared step counts to chunked multi-call extraction
alone, so the four- to nine-fold chunked-versus-oracle ratio
elsewhere in this paper is a conservative under-report of the
multiplicity-driven inflation rather than evidence of it. PAC is
therefore presented as a useful structure-preserving baseline and as
an illustration of the methodological points above, not as an
empirical winner.

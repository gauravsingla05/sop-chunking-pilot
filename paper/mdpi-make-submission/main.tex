...
empirical evidence that LLM-as-oracle chunker evaluation is not
prompt-portable across model families. On a 26-document public
US-government pilot (a public-domain proxy for the motivating
industrial-SOP domain), under a whole-document oracle prompt that
returns non-empty step graphs on 26 of 26 documents under Gemini~Flash
the same prompt returns empty graphs on 11 of 26 under OpenAI
gpt-4o-mini and on 22 of 26 under the larger gpt-4o, with mean steps
per document collapsing from 14.8 (Gemini) to 6.7 (gpt-4o-mini) to
2.4 (gpt-4o) and the precondition signal collapsing from 26/26 to
9/26 to 1/26. Because conservatism scales \emph{with} OpenAI model
capability, the OpenAI behaviour is not a degenerate-judge calibration
issue at a single checkpoint; it is a property of how the two model
families respond to the identical oracle prompt. The broader
implication is that chunkers cannot be reliably ranked under any
single-LLM silver-standard oracle without human anchoring. As a
secondary observation within this scope, the rank order of five
chunkers on step-count fidelity also changes between Gemini~Flash
and gpt-4o-mini on the 15 documents where both produce a non-empty
oracle graph (paired non-parametric bootstrap, $n=15$, 2{,}000
resamples: rank-1 chunker differs across extractors in 93\% of
resamples). The pilot is under-powered (paired
...
the LLM-as-a-judge literature has documented systematic biases in
LLM-based evaluators---position bias, verbosity preference, and
self-enhancement (a tendency to favour outputs from the same model
family)\cite{zheng2023judging,panickssery2024selfpref}---but this
literature has not been brought to bear on the evaluation of
upstream-pipeline components such as chunkers, where the evaluator's
judgements determine which chunker ``wins''. Together these gaps
motivate a chunking evaluation that targets the structural job a
chunker performs on procedural text, and a controlled inspection of
how the LLM-judge biases established for response-quality evaluation
manifest in chunker rankings.
...
winner. The corpus manifest, all three oracle runs (Gemini~Flash,
gpt-4o-mini, and the thoroughness-matched gpt-4o oracle pass), the
matched-prompt multiplicity-isolation data, the pipeline, and the
reference PAC implementation are released so that a hand-annotated
gold subset and a prompt-symmetric re-evaluation can be conducted on
the same documents.
...
paraphrase. Adapting $P_k$ or WindowDiff to this regime is non-trivial in a
specific way: both metrics are defined over a partition of $T$ token
positions and ask, for a window of width $k$, whether the number of
within-window segment boundaries in the hypothesis matches the
reference. In the present setting there is no shared token-position
axis between hypothesis and reference, because the per-chunk extractor
paraphrases ``step~7'' into a sentence whose token positions in the
original document text are not the same as the oracle's step-7 token
positions, and the step set is matched by cosine similarity on
sentence embeddings rather than by boundary location. A faithful
adaptation would need to define a window over the aligned-node
sequence rather than over the source-text token sequence and re-derive
the metric's expected behaviour under chance, which we have not done.
Step-count fidelity is the simpler instrument that nonetheless
captures the count-discrepancy axis those metrics were designed to
expose, at the cost of not localising where the misalignment falls.
We position it as deliberately narrower than $P_k$ or WindowDiff and
note the gap explicitly so that subsequent work can decide whether to
do the adaptation or to use count fidelity as-is.
...
\paragraph{Worked example contrasting the three step-level metrics.}
A reviewer might reasonably ask whether step precision (below) is not
already a sufficient companion to $\tau$ for the over-segmentation
pathology, and whether step-count fidelity is therefore redundant.
Table~\ref{tab:metric-comparison} shows the three metrics on three
synthetic chunkers against a gold of 10 steps. Both step precision
and step-count fidelity catch the over-segmenting chunker (row~3);
only step-count fidelity catches the under-segmenting chunker
(row~2), which $\tau$ misses (perfect order on the 5 aligned steps)
and step precision misses (100\% of declared steps align). The
distinguishing case for step-count fidelity over \{$\tau$, precision\}
is therefore under-segmentation, in which a chunker drops content
silently while every metric except count fidelity scores it well.
This is a narrower contribution than ``catches the pathology'' would
suggest, and the present paper makes that scope explicit.

\begin{table}[ht]
\centering
\caption{Three synthetic chunker outputs against a gold of $n^{*}=10$
steps, with the three step-level metrics. Under-segmentation
(row~2) is the case in which step precision is uninformative ($1.0$)
while step-count fidelity correctly flags the count mismatch ($0.5$).}
\label{tab:metric-comparison}
\small
\begin{tabular}{lccccc}
\toprule
Chunker behaviour & $\hat{n}$ & $\hat{n}$ aligned to gold &
$\tau$ & Step precision & Step-count fidelity \\
\midrule
Balanced (drops two)        & 10 & 8 & 0.78 & 0.80 & 1.00 \\
Under-segmenting (precise)  &  5 & 5 & 1.00 & 1.00 & 0.50 \\
Over-segmenting (noisy)     & 30 & 8 & 0.71 & 0.27 & 0.33 \\
\bottomrule
\end{tabular}
\end{table}
...
completed in approximately two hours of wall clock.

The most robust empirical finding from this exercise is the
\emph{family-prompt non-portability} reported in
Table~\ref{tab:oracle-by-family} and discussed in the next paragraph:
under an identical oracle prompt designed for the Gemini-Flash
oracle, the OpenAI family produces oracle graphs that are
near-unusable. This finding holds regardless of any chunker
comparison and undercuts the assumption that a single-LLM
silver-standard oracle is portable across model families. The
rank-order changes on the 15 documents where both extractors did
return non-empty oracle graphs (Table~\ref{tab:robust}) are reported
secondarily; they document a paired property of the two oracles on
that slice, but their interpretation is bounded by the family-prompt
non-portability above, because the OpenAI side of that comparison
is itself a strongly-conservative judge under the present prompt.

\emph{Family-prompt non-portability under an identical oracle
prompt.} Under the same whole-document oracle prompt that produces
non-empty graphs on every one of the 26 documents under Gemini Flash,
gpt-4o-mini returns an empty graph on 11 of 26 documents and a
non-empty graph with zero preconditions on every one of the
remaining 15; gpt-4o (the larger, more capable OpenAI model) is
\emph{more} conservative still, returning an empty graph on 22 of
26 documents and at least one precondition on only 1 of 26 documents
overall (Table~\ref{tab:oracle-by-family}). Because the conservatism
scales \emph{with} model capability inside the OpenAI family
(stronger model, sparser oracle), it cannot be attributed to one
under-calibrated checkpoint; it is a property of how the OpenAI
family responds to the present oracle prompt versus how the Gemini
family does. This in turn means that any LLM-as-oracle protocol for
chunker evaluation in this regime is not prompt-portable across model
families without per-family prompt re-engineering, and the assumption
that a Gemini-prompted oracle and an OpenAI-prompted oracle are
interchangeable silver standards is empirically false on this corpus.
The result is robust independent of any downstream rank comparison:
when 42\% to 85\% of documents return empty oracle graphs, the
oracle is not a usable silver standard regardless of whether the
chunkers it rates fall in one rank order or another.

A natural and important objection to this finding is that the
``empty graphs'' from the OpenAI family might not be substantive
empty judgments at all but instruction-following failures---refusals,
truncations, or malformed non-JSON output that the parser converts
into an empty record. Inspection of the released oracle outputs
refutes this: every empty record under both gpt-4o-mini and gpt-4o
is a well-formed JSON object conforming to the requested schema
(\texttt{\{"steps":[],"preconditions":[],"constraints":[],"edges":[]\}}),
each is produced with the same 23 output tokens, none carries an
error flag in the released metadata, and none truncates or refuses.
The OpenAI family is therefore making the substantive judgement
``this document contains no extractable steps, preconditions,
constraints, or edges,'' formatted exactly as the prompt requires.
The collapse is a substantive sparsity choice, not a format-compliance
failure, which strengthens rather than weakens the
non-portability claim above.

\emph{Rank-order changes on the 15-document paired slice.}
Table~\ref{tab:robust} reports per-chunker means on the 15 documents
where both oracles returned a non-empty graph (the apples-to-apples
paired comparison); the OpenAI side of the table is also released
as Supplementary Table~S3. On step-count fidelity, the rank order
of the five chunkers differs between extractors (Gemini Flash:
PAC $>$ semantic $>$ layout-aware $>$ recursive $>$ fixed-size;
gpt-4o-mini: semantic $>$ fixed-size $>$ PAC $>$ layout-aware $>$
recursive). On Kendall's $\tau$ the ranking also shifts. The
precondition-recall comparison is not defined under gpt-4o-mini
because that oracle returned zero preconditions on all 15
documents; following the Methods convention the column is reported
as undefined rather than coerced to $1$ by the $0/0$ default. This
rank-order result is reported secondarily because its interpretation
inherits the family-prompt non-portability caveat above: the OpenAI
ranker on this slice is the same oracle that fails on 42\% of the
26-document pilot. The bootstrap quantification below characterises
how stable the rank order is across document resamples on the
15-document slice, conditional on accepting the OpenAI ranker on
those 15 documents as informative; it does not address the
family-prompt non-portability concern, which is what
Table~\ref{tab:oracle-by-family} establishes.
...
empty-graph and zero-precondition behaviour reported for OpenAI is
not a degenerate-judge calibration issue at the gpt-4o-mini level:
...
therefore a property of how the two families respond to the present
oracle prompt, and the assumption that a single oracle prompt is
portable across model families is empirically false on this corpus.}
...
\begin{table}[ht]
\centering
\caption{Rank-order on the 15 documents with non-empty oracle under
both Gemini Flash and gpt-4o-mini, reported as a secondary observation
within the family-prompt non-portability scope established by
Table~\ref{tab:oracle-by-family}. Means are paired at the document
level; \emph{count} is step-count fidelity, $\tau$ is Kendall's
$\tau$. Best per extractor in \textbf{bold}; the bold is visual
orientation only, the cross-extractor non-replication is established
by rank change (and the bootstrap quantification in the text), not
by a paired significance test. The OpenAI side of this table inherits
the strong-conservatism caveat of Table~\ref{tab:oracle-by-family}.}
\label{tab:robust}
\begin{tabular}{lcccc}
\toprule
...
To quantify how robust the rank order is to document resampling at
this small slice size, a paired non-parametric bootstrap (2{,}000
resamples over the 15 shared documents) was run on the per-document
step-count-fidelity values in Table~\ref{tab:robust}. Under Gemini
Flash PAC's bootstrap-mean step-count fidelity is 0.517 with a 95\%
CI of $[0.39, 0.65]$, semantic 0.432~$[0.33, 0.54]$, and layout-aware
0.383~$[0.27, 0.51]$; PAC is the rank-1 chunker in 93\% of resamples
and semantic is rank-2 in 81\%. Under gpt-4o-mini on the \emph{same}
15 documents PAC drops to 0.277~$[0.17, 0.40]$ and is rank-1 in only
9\% of resamples (mode rank: 5, in 36\% of resamples), while semantic
is rank-1 in 50\% and fixed-size is rank-2 in 29\%. The rank-1
chunker differs between the two extractors in 93\% of paired
bootstrap resamples (1{,}858 of 2{,}000). Conditional on accepting
the OpenAI ranker on the 15 documents where it returned a non-empty
oracle, the cross-extractor rank flip is not a noise artefact of the
small slice; conditional on the family-prompt non-portability of
Table~\ref{tab:oracle-by-family}, the rank flip is downstream of an
oracle that does not function reliably across families to begin with.

The honest synthesis of the cross-extractor evidence is that LLM-as-oracle
chunker evaluation in this regime is not prompt-portable across
model families, and that ``a calibrated cross-extractor test under
a single shared oracle prompt across vendors'' is not available to
this work. What can be supported on this evidence is the broader
methodological claim that chunkers cannot be reliably ranked under
any single-LLM silver-standard oracle without human anchoring; the
narrower claim that one specific Gemini-vs-gpt-4o-mini rank order
flips on the 15-document slice is reported as a secondary
...
steps on six of the eight documents (11 and 7 steps on the two it did
fire on; mean $9.0$ over those two, $0$ over the other six, $2.25$
over all eight; see Supplementary Table~S4 for per-document values).
The conservative prompt was originally rejected as the oracle
precisely because of this empty-graph mode. The 6.3$\times$ figure should therefore be
...
\paragraph{What a practitioner can take away today.} Before the
limitations, three concrete recommendations follow from this work and
do not depend on the open empirical questions discussed below.
\emph{(i)~Treat chunking as a controlled, auditable pipeline stage
rather than a neutral preprocessing detail}: any future SOP-automation
pipeline should pin the chunker version, ship with a small acceptance
set of representative SOPs with their hand-checked step graphs, and
re-evaluate against that set whenever the chunker or extractor
changes. \emph{(ii)~Pair any order-of-steps metric with a count-fidelity
metric}: as Table~\ref{tab:metric-comparison} makes explicit, an
order-only protocol can reward a chunker that over-segments and a
precision-only protocol can pass a chunker that under-segments;
counting the steps as a companion check disambiguates both. \emph{(iii)~Do
not rely on a single LLM as the oracle for chunker selection}: this
paper's instability result (Section~\ref{sec:robustness}, Tables~6 and~7)
shows that two LLM families differ enough on the same prompt that
rankings reorder; whether the disagreement is genuine inter-judge
disagreement or one family failing to follow the prompt, the practical
implication is the same---hand-anchor the evaluation at the chunker-
selection step. The PAC implementation released here is offered as a
worked example of a structure-preserving chunker that practitioners
can audit and adapt, not as a deployment default.
...
confounded in the present setup. This is the chunker-evaluation
analogue of the self-enhancement bias documented in the LLM-as-a-judge
literature for response-quality
evaluation\cite{zheng2023judging,panickssery2024selfpref}, in which an
LLM judge rates outputs from its own family more favourably; here the
favouritism manifests as the same-model oracle scoring most highly the
chunker whose output regime resembles its own whole-document input
regime. This is named explicitly: count
...
fragmentation visible. Second, an oracle-by-family comparison under an identical
whole-document oracle prompt shows that the prompt is not portable
across LLM families: Gemini~Flash returns non-empty step graphs on
26 of 26 documents, gpt-4o-mini on 15 of 26 (and 0/15 preconditions),
and the larger gpt-4o on only 4 of 26 (and 1/26 preconditions). The
collapse scales with OpenAI model capability, so it is not a
checkpoint calibration issue; it is a family-prompt interaction
that makes the single-LLM silver-standard oracle a brittle
foundation for chunker evaluation regardless of which chunker is
rated best on the subset where the oracle does return a non-empty
graph. The synthesis is that chunkers cannot be reliably ranked
under any single-LLM silver-standard oracle in this regime without
human anchoring. The 15-document rank-flip between Gemini and
gpt-4o-mini (rank-1 chunker differs in 93\% of paired bootstrap
resamples) is a corollary of this scope, not its centre.
...
The corpus manifest, all three oracle runs (Gemini~Flash,
gpt-4o-mini, and the thoroughness-matched gpt-4o oracle pass), the
pipeline, the matched-prompt multiplicity-isolation outputs, the
...
\supplementary{The following supplementary tables are released with
the manuscript: Supplementary Table~S1 (chunk counts per (document,
...
the 15-document slice), and Supplementary Table~S4 (matched-prompt
multiplicity-isolation data on the eight smallest documents that
underlie the idempotency control). The corpus manifest, all three
oracle runs (Gemini~Flash, gpt-4o-mini, and the thoroughness-matched
gpt-4o oracle pass), the matched-prompt multiplicity-isolation
outputs, the pipeline source, and the reference PAC implementation
are available in the project repository (see Data Availability
Statement).}
...
analysis CSVs for both per-chunk extractor runs (Gemini~Flash under
\texttt{data/runs/pilot-001-gemini/} and OpenAI gpt-4o-mini under
\texttt{data/runs/pilot-002-openai/}) plus the standalone
thoroughness-matched gpt-4o oracle pass under
\texttt{data/runs/pilot-003-openai-gpt4o-oracle/} that underlies
Table~\ref{tab:oracle-by-family}; (iv) Supplementary Tables~S1--S4
as CSV files at the repository root; and (v) the LaTeX source for the
present submission. The same repository carries the matched-prompt
multiplicity-isolation outputs under

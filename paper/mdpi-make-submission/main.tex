...
oracle step counts. The four- to nine-fold ratio is multiplicity-driven: a matched-prompt
control (Section~\ref{sec:robustness}, ``Idempotency check'')
attributes 9 to 16-fold inflation to chunked multi-call extraction
alone with the prompt held conservative on both passes, so the
chunked-versus-thorough-oracle ratio shown here is a conservative
under-report of the multiplicity-driven inflation rather than evidence
of it.}
\label{fig:overseg}
\end{figure}
...
prompts on the same whole document).

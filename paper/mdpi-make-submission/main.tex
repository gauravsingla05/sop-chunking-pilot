...
% =============================================================
\Title{Step-Count Fidelity as a Companion Metric for Chunking
Procedural Text, and the Instability of LLM-Judged Chunker Evaluation:
A Pilot on Industrial Standard-Operating-Procedure Automation}
...
methods-and-pilot paper with two contributions and one worked example.
The first contribution is to pair step-count fidelity with Kendall's
$\tau$ as companion metrics for chunking procedural text, on the
observation that $\tau$ is computed only over aligned gold steps and
is therefore blind to spurious extra steps, while a count-fidelity
metric is not. The second is direct empirical evidence that
LLM-as-oracle chunker rankings are unstable. On a 26-document public
US-government pilot (OSHA, NRC, EPA, CDC/NIOSH; a public-domain proxy
...
presence under Gemini~Flash (26 of 26 documents) to universal absence
under gpt-4o-mini (0 of 15). A natural concern is that gpt-4o-mini
might be a degenerate second judge rather than a peer to Gemini Flash;
a thoroughness-matched re-run of the oracle with gpt-4o under the
identical prompt refutes that hypothesis empirically by producing an
\emph{even more} conservative oracle (22 of 26 documents return empty
step graphs and only 1 of 26 has any precondition under gpt-4o,
compared with 11 of 26 and 9 of 26 under gpt-4o-mini). The OpenAI
family is consistently more conservative than the Gemini family under
the same prompt regardless of OpenAI model size, so the cross-extractor
instability is a property of how the two model families respond to
the oracle prompt, not of a single under-calibrated checkpoint. The evaluation oracle is itself an LLM,
no human-annotated reference is included, and the pilot is
underpowered: a retrospective paired-test power analysis indicates
that detecting the closest pairwise effect against the semantic
baseline at $\alpha=0.05$ and power 0.8 would require approximately
$n=156$ documents. Under this regime, the absolute fidelity numbers
reported in the body of the paper are not interpretable in level, and
even the rank moves between extractors. A transparent rule-based
position-aware chunker (PAC) is presented as a worked structure-
preserving baseline that exists to make the metric demonstrable; it is
not an empirical winner. The corpus manifest, both extractor runs, the
...
gold steps both score $0.5$. This symmetry is what step-count fidelity
adds over step precision (below). Step precision penalises only
over-generation: a chunker that under-segments but whose declared
steps all align to gold steps scores $1.0$ on step precision while
having missed real content; step-count fidelity flags this as a count
mismatch. The two metrics are therefore not redundant. Under
LLM-as-oracle evaluation as in this pilot, the level of step-count
fidelity is partly determined by chunk count rather than by structural
correctness (Section~\ref{sec:results}, ``Robustness checks''), but
its qualitative diagnostic value as the symmetric companion to step
precision and to $\tau$ survives that caveat.
...
A retrospective power calculation makes this concrete. The observed
Cohen's $d_z$ for PAC vs.\ each baseline on step-count fidelity on
the full 26-document corpus is 0.34 against fixed-size, 0.42 against
recursive, 0.22 against semantic, and 0.28 against layout-aware. For
a paired two-sided test at $\alpha=0.05$ and power 0.8, the corpus
sizes required to detect those observed effects are approximately
$n=70$, $n=46$, $n=156$, and $n=104$ respectively. The closest
competitor is semantic chunking ($d_z=0.22$); detecting that effect
at conventional significance levels would require approximately
$n\approx156$ documents on a paired protocol, six times the present
pilot. The uncorrected $p=0.046$ against recursive and $p=0.040$
against fixed-size should therefore be read as the modest signals of
an under-powered pilot, not as near-significant findings; the
empirical contribution of the paper carries on the framework and on
the instability result of Section~\ref{sec:robustness}, not on the
pairwise Gemini comparisons.
...
\toprule
& Name & Numerator & Denominator & Document set & Prompts (num/denom) \\
\midrule
...
$\alpha=0.05$. A retrospective power calculation
(Section~\ref{sec:stats-power}) shows that the closest baseline
comparison (PAC versus semantic chunking) would require
approximately $n\approx156$ documents to detect the observed effect
size at conventional power; the present pilot is six-fold
under-powered for that comparison. No PAC superiority claim is made:
the empirical contribution of the paper is the framework and the
...
The pilot is six-fold under-powered for the closest pairwise
comparison (Section~\ref{sec:stats-power}); the count-fidelity metric

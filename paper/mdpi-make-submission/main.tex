\documentclass[journal=make,article,submit,oneauthor,pdftex]{Definitions/mdpi}
...
% =============================================================
\Title{Position-Aware Chunking for Industrial
Standard-Operating-Procedure Automation: Preserving Step Granularity
and Preconditions in LLM-Extracted Workflows}
...
the source document, but chunking strategies designed for
retrieval-augmented question answering treat document order as
incidental while SOPs invert this assumption: step sequence and
precondition--step adjacency are load-bearing. This work formalises
three structural-fidelity properties for chunking procedural text
(step-order preservation, precondition adjacency, safety-constraint
attachment), introduces step-count fidelity as the counterpart to
Kendall's $\tau$ that exposes the over-segmentation pathology under
which order-only metrics reward chunkers that fragment the source into
a long noisy step list, and reports a 26-document pilot on public
US-government SOPs comparing five chunkers (fixed-size, recursive,
semantic, layout-aware, and a rule-based position-aware chunker, PAC)
while holding the LLM extractor and prompt constant. Two findings are
robust across the design: the four widely used baseline chunkers
over-segment SOPs four- to nine-fold relative to a whole-document
oracle on both the full corpus and a 20-document procedural-only
subset, and the chunker ranking is partly extractor-dependent, as
demonstrated by a cross-extractor robustness check (Gemini~Flash
versus OpenAI gpt-4o-mini) in which the step-count-fidelity rank
order changes. PAC is presented as an existence proof that a
structure-preserving chunker can avoid the over-segmentation pathology
by construction; PAC leads on step-count fidelity under the
Gemini-based pilot (uncorrected $p=0.046$ and $0.040$ against the two
simplest baselines, neither surviving family-wise correction across
the 24 paired tests run), but the lead does not replicate under
gpt-4o-mini. The same-model coupling between extractor and oracle is
named as the deepest remaining confound and a hand-annotated gold
subset is required to settle the empirical question.}
...
closed, skip steps 12--15'' is a hard conditional. ``Wear personal
protective equipment (PPE) before opening the cabinet'' must travel with
the cabinet-opening step or the operator may be injured. A chunker that
scores well on RAG can quietly split a numbered step, separate a
precondition from its target, or fragment a conditional branch, and the
generated workflow inherits the corruption silently, indistinguishable
downstream from a model hallucination\cite{ji2023hallucination} but
with the same safety consequence.
...
which would survive family-wise correction across the 24 paired tests
...
present in the chunk. An asymmetry between this per-chunk prompt and
the whole-document oracle prompt is noted: the oracle is instructed
to be thorough (see ``Oracle ground truth''), so prompt wording alone
can contribute to a count gap between $\hat{G}(D)$ and $G^{*}(D)$ that
is not chunker-attributable; this is treated as a confound in the
Discussion. We use Gemini's free tier for the pilot. The
...
de-duplicated by embedding similarity (cosine threshold 0.78, higher
than the metric-alignment threshold because chunk overlap usually
contains verbatim repeats). Preconditions and constraints are
de-duplicated similarly. Edges are unioned, with endpoint ids remapped
to canonical ids and danglers dropped. \texttt{Precedes} edges are
synthesised from the merged step order so the graph is connected even
when the per-chunk extractor omitted them.
...
public-domain works, so the corpus manifest (source URLs, SHA-256
hashes, and per-document metadata) is released without restriction.
...
general administration), and a science-advisory report. We report both slices because the full
corpus shows what the surface filter admits and the procedural subset
shows the chunkers' behaviour on the document type the work is
ultimately about.
...
interpreting significance. Because 24 paired tests are run across two
slices and six metrics, the probability of at least one false positive
at $\alpha = 0.05$ exceeds 0.50 under a strict family-wise null. The
...
filter on the basis of language and surface keyword counts but lack
genuine step structure on inspection.
...
highest mean. Two of the 24 paired tests run in the pilot reach
...
PAC's advantage on count fidelity and precondition recall does not come
at the cost of step ordering. On the full corpus, the per-chunker
Kendall's~$\tau$ means cluster within a narrow band (fixed 0.221,
recursive 0.255, semantic 0.238, layout 0.161, PAC 0.215). No paired
comparison between PAC and a baseline on $\tau$ reaches statistical
significance, and the direction varies (PAC has a slightly higher
$\tau$ than layout-aware on both slices and slightly lower than
recursive). Read alongside the count-fidelity result, this means PAC
matches the baselines' ordering quality on the steps that are
recovered, while producing a step list whose size matches the source.
...
threshold, metrics) constant. The rerun was 2,290 API calls and completed in approximately
...
family-wise correction across the 24 paired tests run in the pilot
...
construction. Precondition recall is more independent of this
mechanical coupling than count fidelity (a precondition can be present
or absent at any chunk granularity) and is therefore the more
informative of the two metrics PAC leads on, though it too is computed
against the same oracle. The cross-extractor robustness check reported
in Results gives direct empirical evidence for this concern: switching
the extractor and oracle to OpenAI gpt-4o-mini changes the
step-count-fidelity rank order on the 15 documents where both oracles
return non-empty graphs (PAC drops to third; semantic chunking leads),
which suggests that the chunker ranking on this pilot is partly a
function of the model used to evaluate the chunks. The clean break is
a hand-annotated gold subset; the pilot results should therefore be
read as motivating but not settling the underlying empirical question.
...
Even uncorrected, only two of 24 paired tests reach $p<0.05$, and
...
This work treats chunking as a load-bearing stage of an LLM
SOP-automation pipeline rather than as a neutral preprocessing step.
Three structural-fidelity properties were formalised for procedural
text: step-order preservation, precondition adjacency, and
safety-constraint attachment. Step-count fidelity was introduced as
the counterpart to Kendall's $\tau$ that exposes the over-segmentation
pathology under which order-only metrics reward chunkers that fragment
the source into a long, noisy step list. On a 26-document pilot of
publicly available US-government SOPs, the four widely used baseline
...
finding that is robust to the choice of extractor and to the
merge-threshold sweep reported in the Robustness checks. A
cross-extractor robustness check (Gemini~Flash versus OpenAI
gpt-4o-mini) shows that chunker ranking under LLM-based evaluation is
partly extractor-dependent, which is direct empirical support for
treating same-model coupling between extractor and oracle as the
deepest remaining confound. The position-aware chunker proposed here
is offered as a transparent rule-based existence proof that a
structure-preserving chunker can avoid the over-segmentation pathology
by construction; it leads on step-count fidelity under the
Gemini-based pilot at uncorrected $p<0.05$ against the two simplest
baselines but the lead does not replicate under gpt-4o-mini and would
not survive a family-wise correction. The corpus manifest, both
extractor runs, the pipeline, and a reference PAC implementation are
released so that the pilot can be extended; the natural next step is
a hand-annotated gold subset that breaks the same-model coupling and
permits a true gold-standard evaluation of PAC and its competitors.
...
\dataavailability{All documents in the pilot corpus are
US-government public-domain works retrievable from the URLs in the
released corpus manifest, which records the source URL, SHA-256
content hash, and per-document metadata for each of the 26 SOPs. The
per-cell extractor outputs, merged document-level graphs, oracle
graphs, and analysis CSVs for both extractor runs (Gemini~Flash and
OpenAI gpt-4o-mini) are released as JSON and CSV files in the project
repository. Supplementary Tables~S1--S3 are provided as CSV files with
the submission.}

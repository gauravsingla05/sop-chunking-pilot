...
To quantify how robust the rank flip is to document resampling at this
small slice size, a paired non-parametric bootstrap (2{,}000 resamples
over the 15 shared documents) was run on the per-document
step-count-fidelity values reported in Table~\ref{tab:robust}. Under
Gemini Flash, PAC's bootstrap-mean step-count fidelity is 0.517 with a
95\% CI of $[0.39, 0.65]$, semantic 0.432~$[0.33, 0.54]$, and
layout-aware 0.383~$[0.27, 0.51]$; PAC is the rank-1 chunker in 93\%
of resamples and semantic is rank-2 in 81\%. Under gpt-4o-mini on the
\emph{same} 15 documents PAC drops to 0.277~$[0.17, 0.40]$ and is
rank-1 in only 9\% of resamples (mode rank: 5, in 36\% of resamples),
while semantic is rank-1 in 50\% and fixed-size is rank-2 in 29\%.
The rank-1 chunker differs between the two extractors in 93\% of
paired bootstrap resamples (1{,}858 of 2{,}000). The cross-extractor
rank flip is therefore not a noise artefact of the small slice; it
is a real, paired property of the two LLM judges on this corpus,
albeit one whose interpretation is bounded by the family-failure
concern reported below.
